...
> 基于 Python、PyWebView (HTML5/CSS3/ES6) 与 OpenCV 的现代化极简易用桌面端可视化自动化操作软件。
...
1. **零门槛极简易懂 · 积木式流程编排**
   - 像读故事一样看懂每一个步骤（如“在屏幕上找到 [登录按钮] 并 单击鼠标左键”、“自动输入文字 '你好'”）。
   - 常用操作一键添加（🎯 找图点击、🖱️ 坐标点击、⌨️ 输入文字、⚡ 快捷键、⏱️ 等待延时、↔️ 鼠标拖拽等）。
   - 步骤卡片支持鼠标直接上下拖拽调序、启用/禁用开关、一键单步即时测试与复制。
   - 新建空白流程时配备 3 步新手向导与一键新手示例载入。

2. **渐进式属性配置 (Progressive Disclosure)**
   - 小白用户只需配置核心项（如按 F7 截取目标图片），高级算法参数（相似度阈值、多尺度自适应、灰度模式、失败策略、前后延时等）默认折叠并配置最佳默认值。
...
│   │       ├── index.html   # 主界面结构
│   │       ├── css/
│   │       │   └── style.css# 暗黑系现代化设计与流畅动效
│   │       └── js/
│   │           ├── icons.js # 矢量 SVG 图标库
│   │           └── app.js   # 响应式状态管理、拖拽排序与交互控制
│   └── utils/
...
2. **截取目标**：点击右侧面板的 **“✂️ 截取目标图片 (F7)”**，拖拽框选要点击的按钮，释放即可完成取样。
