...
- **端口连线**：从节点输出端口拖出连线，按网格正交路由自动折线，支持 True / False 双出口分支。
...
- 12 类动作一键添加，语义化命名（如「找图点击」「输入文字 '你好'」）。
...
- **条件分支**：判断目标图像「存在 / 不存在」，分别执行继续、跳转、跳过 N 步或停止。
- **失败分支**：步骤执行失败时按画布 False 端口跳转，优先于全局失败策略。
- **自定义跳转**：任意步骤可指定下一步跳转到第 N 步，实现循环体。
- **失败策略**：失败即停 / 忽略继续 / 自动重试（可配重试次数与间隔）。
...
4. **编排连线**（可选）：从节点右侧端口拖出连线到下一步节点，或连接条件节点的 True / False 出口做分支。点击工具栏「一键智能排版」自动对齐。
...
引擎共注册 **13** 类动作处理器，动作面板开放其中 **12** 类可直接新建。
...
| 条件分支 | `condition` | 判断目标图像存在 / 不存在，分流到 True / False 出口。**引擎与属性面板均已支持，由 `.pmflow` 载入或示例流程提供，动作面板新建入口规划中** |

### 扩展预留

| 动作 | 标识 | 状态 |
| --- | --- | --- |
| OCR 文字识别点击 | `ocr_click` | 已挂载插件骨架，需自行安装 OCR 引擎（如 RapidOCR / Tesseract）后接入生效 |
| 窗口激活 | `window_activate` | 枚举已定义，尚未实现 |
...
| 条件分支连线 | 条件节点有 **True / False** 两个输出端口，分别连到不同后继节点 |
...
**纯浏览器调试模式**：访问 `http://localhost:5173/?mock=1` 会注入一份示例流程（含条件分支节点），无需启动 Python 后端即可调试画布、连线与标签渲染。
